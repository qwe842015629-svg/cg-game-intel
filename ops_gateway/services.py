...
def discover_missing_google_play_package_ids(*, limit: int = 40) -> dict[str, Any]:
    ...
    for seed_url in _GOOGLE_PLAY_DISCOVERY_SEEDS:
...
def run_daily_robot_full_cycle(
    *,
    ...
    _emit("discovery.started", {"limit": max(20, normalized_import_limit * 2)})
    discovery = discover_missing_google_play_package_ids(limit=max(20, normalized_import_limit * 2))
    _emit(
        "discovery.completed",
        {
    ...
    import_result = run_google_play_import_automation(
        ...
        overwrite_existing=False,
        limit=normalized_import_limit,
        auto_discover_missing=not bool(discovery.get("package_ids")),
