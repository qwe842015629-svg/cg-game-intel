...
def _stall_timeout_minutes(runtime_config: dict[str, Any] | None = None) -> int:
    running_timeout = _running_timeout_minutes(runtime_config)
    configured = _safe_int(getattr(settings, "OPS_GATEWAY_AUTORUN_STALL_TIMEOUT_MINUTES", 45), 45)
    return max(10, min(running_timeout, configured))
...
def _is_stale_running(row: DailyRobotRun, *, now_dt, runtime_config: dict[str, Any] | None = None) -> bool:
    ...
    stall_seconds = _stall_timeout_minutes(runtime_config) * 60
    if started_at and (now_dt - started_at).total_seconds() >= max_seconds:
        return True
    last_heartbeat = updated_at or started_at
    if last_heartbeat and (now_dt - last_heartbeat).total_seconds() >= stall_seconds:
        return True
    return not bool(started_at or updated_at)
...
def _mark_stale_daily_runs(*, now_dt, runtime_config: dict[str, Any] | None = None) -> int:
    ...
    stall_timeout_seconds = _stall_timeout_minutes(runtime_config) * 60
    stale_before = now_dt - timedelta(seconds=timeout_seconds)
    stall_before = now_dt - timedelta(seconds=stall_timeout_seconds)
    ...
    for row in rows:
        ...
        if (
            ...
            runtime_stale = started_at is not None and started_at <= stale_before
            heartbeat = updated_at or started_at
            heartbeat_stale = heartbeat is not None and heartbeat <= stall_before
            if started_at is None and updated_at is None:
                is_stale = True
            else:
                is_stale = runtime_stale or heartbeat_stale
        if not is_stale:
            continue
