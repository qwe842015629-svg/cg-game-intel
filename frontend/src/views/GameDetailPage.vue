<template>
  <div class="min-h-screen bg-background text-foreground pb-20">
    ...
    <div v-else-if="error" class="container mx-auto px-4 py-12 text-center">
      ...
      <router-link :to="localizedPath('/games')" class="text-primary hover:underline">
        {{ t("gameDetailPage.actions.backToGames") }}
      </router-link>
    </div>
    ...
    <div v-else class="container mx-auto px-4 py-6 max-w-7xl">
      <nav class="flex items-center text-sm text-muted-foreground mb-6" v-motion-reveal="{ y: 14 }">
        <router-link :to="localizedPath('/')" class="hover:text-primary transition-colors">
          {{ t("gameDetailPage.breadcrumbs.home") }}
        </router-link>
        <span class="mx-2">/</span>
        <router-link :to="localizedPath('/games')" class="hover:text-primary transition-colors">
          {{ t("gameDetailPage.breadcrumbs.games") }}
        </router-link>
        <span class="mx-2">/</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden" v-motion-reveal="{ y: 24 }">
            <div v-if="game.banner_image_url" class="h-48 w-full relative">
              ...
              <img
                ...
                :alt="t('gameDetailPage.bannerAlt')"
              />
            </div>
            ...
            <div class="p-6 relative z-20 flex flex-col md:flex-row gap-6" :class="{ 'mt-[-4rem]': game.banner_image_url }">
              <div class="flex-shrink-0 mx-auto md:mx-0">
                <div
                  class="w-32 h-32 md:w-40 md:h-40 rounded-xl overflow-hidden border-4 border-card shadow-lg bg-muted"
                  v-tilt="{ max: 3.2, scale: 1.01 }"
                >
                  <img
              ...
              <div class="flex-1 text-center md:text-left space-y-3">
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-2">
                  <span
                    v-if="game.is_hot"
                    class="px-2 py-0.5 rounded text-xs font-medium bg-red-500/10 text-red-500 border border-red-500/20"
                  >
                    🔥 {{ t("gameDetailPage.tags.hot") }}
                  </span>
                  <span
                    v-if="game.is_recommended"
                    class="px-2 py-0.5 rounded text-xs font-medium bg-blue-500/10 text-blue-500 border border-blue-500/20"
                  >
                    ⭐ {{ t("gameDetailPage.tags.recommended") }}
                  </span>
                  <span class="px-2 py-0.5 rounded text-xs font-medium bg-secondary text-secondary-foreground">
                    {{ game.category_name || t("gameDetailPage.region.global") }}
                  </span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-muted-foreground max-w-md mx-auto md:mx-0">
                  <div class="flex items-center gap-2">
                    ...
                    <span>
                      {{ t("gameDetailPage.meta.developer") }}{{ game.developer || t("gameDetailPage.meta.unknownDeveloper") }}
                    </span>
                  </div>
                  <div class="flex items-center gap-2">
                    <i class="fas fa-globe w-4"></i>
                    <span>{{ t("gameDetailPage.meta.region") }}{{ displayRegion }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <i class="fas fa-mobile-alt w-4"></i>
                    <span>
                      {{ t("gameDetailPage.meta.platform") }}{{ game.platform || t("gameDetailPage.meta.unknownPlatform") }}
                    </span>
                  </div>
                  <div class="flex items-center gap-2">
                    <i class="fas fa-server w-4"></i>
                    <span>{{ t("gameDetailPage.meta.server") }}{{ game.server_name || t("gameDetailPage.meta.allServers") }}</span>
                  </div>
                </div>
            ...
            <div class="px-6 pb-6 pt-2 border-t border-border mt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div class="text-sm text-muted-foreground">
                <span class="text-primary font-bold">{{ game.view_count ?? 0 }}</span>
                {{ t("gameDetailPage.viewingCount") }}
              </div>
              <div class="flex gap-3 w-full sm:w-auto">
                <button
                  @click="contactSupport"
                  v-magnetic="{ strength: 0.2, max: 11 }"
                  class="flex-1 sm:flex-none px-6 py-2.5 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-all font-medium shadow-md shadow-primary/20 flex items-center justify-center gap-2"
                >
                  <i class="fas fa-comment-dots"></i> {{ t("gameDetailPage.actions.contactRecharge") }}
                </button>
                <button
                  class="px-4 py-2.5 bg-secondary text-secondary-foreground rounded-lg hover:bg-secondary/80 transition-all border border-border"
                  :aria-label="t('gameDetailPage.actions.share')"
                  :title="t('gameDetailPage.actions.share')"
                >
        ...
        <div class="lg:col-span-4 space-y-6">
          <div
            class="bg-card rounded-xl border border-border p-5 shadow-sm"
            v-motion-reveal="{ delay: 70, y: 16 }"
            v-tilt="{ max: 2.8, scale: 1.004 }"
          >
            <h3 class="font-bold text-foreground mb-4 flex items-center gap-2">
              <i class="fas fa-shield-alt text-primary"></i> {{ t("gameDetailPage.serviceGuarantee.title") }}
            </h3>
            <div class="space-y-4">
              <div class="flex gap-3">
                ...
                <div>
                  <div class="font-medium text-sm">{{ t("gameDetailPage.serviceGuarantee.official.title") }}</div>
                  <div class="text-xs text-muted-foreground">{{ t("gameDetailPage.serviceGuarantee.official.desc") }}</div>
                </div>
              </div>
              <div class="flex gap-3">
                ...
                <div>
                  <div class="font-medium text-sm">{{ t("gameDetailPage.serviceGuarantee.fast.title") }}</div>
                  <div class="text-xs text-muted-foreground">{{ t("gameDetailPage.serviceGuarantee.fast.desc") }}</div>
                </div>
              </div>
              <div class="flex gap-3">
                ...
                <div>
                  <div class="font-medium text-sm">{{ t("gameDetailPage.serviceGuarantee.support.title") }}</div>
                  <div class="text-xs text-muted-foreground">{{ t("gameDetailPage.serviceGuarantee.support.desc") }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gradient-to-br from-primary/10 to-transparent rounded-xl border border-primary/20 p-5" v-motion-reveal="{ delay: 120, y: 16 }">
            <h3 class="font-bold text-foreground mb-2">{{ t("gameDetailPage.contactCard.title") }}</h3>
            <p class="text-sm text-muted-foreground mb-4">{{ t("gameDetailPage.contactCard.desc") }}</p>
            <button
              @click="contactSupport"
              v-magnetic="{ strength: 0.18, max: 10 }"
              class="w-full py-2 bg-card border border-border rounded-lg text-sm font-medium hover:bg-muted transition-colors"
            >
              {{ t("gameDetailPage.actions.contactConsult") }}
            </button>
          </div>

          <div class="bg-card rounded-xl border border-border p-5 shadow-sm" v-motion-reveal="{ delay: 160, y: 16 }">
            <h3 class="font-bold text-foreground mb-4">{{ t("gameDetailPage.announcements.title") }}</h3>
            <ul class="space-y-3 text-sm">
              <li class="text-muted-foreground hover:text-primary cursor-pointer truncate">• {{ t("gameDetailPage.announcements.item1") }}</li>
              <li class="text-muted-foreground hover:text-primary cursor-pointer truncate">• {{ t("gameDetailPage.announcements.item2") }}</li>
              <li class="text-muted-foreground hover:text-primary cursor-pointer truncate">• {{ t("gameDetailPage.announcements.item3") }}</li>
            </ul>
          </div>
        </div>
      </div>
...
const tabs = computed(() => [
  { id: "details", label: t("gameDetailPage.tabs.details") },
  { id: "topup", label: t("gameDetailPage.tabs.topup") },
]);
...
const resolveImageUrl = (url: string) => {
  ...
  if (normalized.startsWith("/media/") || normalized.startsWith("/static/")) return `${BACKEND_ORIGIN}${normalized}`;
  if (normalized.startsWith("media/")) return `${BACKEND_ORIGIN}/${normalized}`;
  if (normalized.startsWith("uploads/")) return `${BACKEND_ORIGIN}/media/${normalized}`;
  if (normalized.startsWith("/uploads/")) return `${BACKEND_ORIGIN}/media${normalized}`;
...
const getGameIconCandidates = (): string[] => {
  ...
  const candidates = [game.value?.icon_image_url, game.value?.icon_external_url, game.value?.image, game.value?.banner_image_url];
...
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
  ...
  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#94a3b8" font-size="${Math.round(
    size * 0.1
  )}" font-family="Arial, sans-serif">${safeLabel}</text>
</svg>`;
...
const sanitizeUrl = (rawUrl: string, allowRelative: boolean = true) => {
  ...
  if (allowRelative && /^(\/|\.\/|media\/|uploads\/)/i.test(url)) {
    return url.replace(/^\.\//, "/");
  }
  return "";
};
...
const renderInlineMarkdown = (line: string) => {
  ...
  while ((match = tokenRegex.exec(line)) !== null) {
    ...
    if (match[1] !== undefined) {
      const altText = escapeHtml(match[1] || t("gameDetailPage.content.imageAlt"));
      const imageUrl = resolveImageUrl(sanitizeUrl(match[2] || "", true));
      if (imageUrl) {
        output += `<img class="gp-image" src="${imageUrl}" alt="${altText}" loading="lazy" onerror="this.style.display='none';" />`;
      } else {
...
const detailsContentHtml = computed(() => renderContent(game.value?.content, t("gameDetailPage.empty.details")));
const topupContentHtml = computed(() => renderContent(game.value?.topup_info, t("gameDetailPage.empty.topup")));
...
const displayRegion = computed(() => {
  ...
  if (!rawRegion || /^(global|all)$/i.test(rawRegion)) {
    return t("gameDetailPage.region.global");
  }
  if (["港台服", "港臺服", "hk/tw", "hk-tw"].includes(rawRegion.toLowerCase())) {
    return t("gameDetailPage.region.global");
  }
...
const syncDocumentTitle = () => {
  const gameTitle = String(game.value?.title || "").trim() || t("gameDetailPage.meta.untitledGame");
  document.title = `${gameTitle} - ${t("siteName")}`;
};
...
const fetchGameDetails = async () => {
  ...
  try {
    ...
    console.error(err);
    error.value = t("gameDetailPage.errors.loadFailed");
  } finally {
    loading.value = false;
  }
};
...
const contactSupport = () => {
  router.push(localizedPath("/customer-service"));
};
