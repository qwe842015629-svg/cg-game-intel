...
class EmailLoginTokenCreateSerializer(TokenCreateSerializer):
    ...
    @staticmethod
    def _resolve_existing_users(field: str, value: str):
        if field == "email":
            return User.objects.filter(email__iexact=value).order_by("-is_active", "id")
        return User.objects.filter(**{field: value}).order_by("-is_active", "id")
    ...
    def validate(self, attrs):
        ...
        for field, value in identity_candidates:
            ...
            for existing_user in self._resolve_existing_users(field, value):
...
class DuplicateSafeSendEmailResetSerializer(SendEmailResetSerializer):
    """Resolve reset-password user safely when duplicated emails exist."""

    def get_user(self, is_active=True):
        identity = str(self.data.get(self.email_field, "") or "").strip()
        if not identity:
            if (
        ...
        lookup = {f"{self.email_field}__iexact": identity}
        users = User._default_manager.filter(is_active=is_active, **lookup).order_by("-is_active", "id")
